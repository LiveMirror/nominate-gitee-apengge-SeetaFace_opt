...
inline void my_matrix_procuct_fun(const float* A, const float* B, float* C, const int n,
	const int m, const int k, bool ta, bool tb)
{
	...
	if (size > 20000000)
	{
		SeetaCudaMath::matrix_procuct_gpu(0, A, B, C, n, m, k);
	}
	else
	{
		SeetaCudaMath::matrix_procuct_cpu(A, B, C, n, m, k);
	}
...
void TEST(FaceRecognizerTest, CudaExtractFeatureWithCrop) {

	seeta::FaceIdentification::set_g_extern_matrix_procuct_fun(my_matrix_procuct_fun);
	...
	if (1.0 - average_sim >  0.01) {
		...
		std::cout << "Test successful!\n CudaExtractFeatureWithCrop time: "
			<< 1000.0 * count / CLOCKS_PER_SEC / img_num << "ms" << std::endl;
	}
	delete[]feat_caffe;
	delete[]feat_sdk;
