...
void ConvNet::Execute() {
  ...
  float * const dst_head = (float *)gMemAllocator->malloc(src_num * dst_size * dst_channels * sizeof(float));
  float * const mat_head = (float *)gMemAllocator->malloc(dst_size * kernel_size * sizeof(float));
  //printf("dst_head_buffer:%d, mat_head_buffer:%d\n", src_num * dst_size * dst_channels, dst_size * kernel_size);
  ...
  //delete[] mat_head;
  //delete[] dst_head;
  gMemAllocator->free(mat_head);
  gMemAllocator->free(dst_head);
